...
export async function refreshMasterData() {
  ...
  await Promise.all([
    ...
    storage.set(K_VEHICLES, vehicles),
  ]);
  return { materials, customers, vendors, vehicles };
}
...
export const cache = {
  ...
  vehicles: () => storage.get<Vehicle[]>(K_VEHICLES, []),
};
